...
"""
function lanbpro(A::Union{Matrix{Tv}, Function}, k::Ti; m=[], n=[], params=[],
                 delta=[], Anorm=[], Uold=[], Bold=[], Vold=[], r0=[],
                 examine_flag=false, elr_flag=true, qflag=false) where {Ti<:Int64, Tv <: Union{Float32, Float64, Complex{Float32}, Complex{Float64}, Quaternion{Float32}, Quaternion{Float64}}}

    # decide A is an operator or matrix
    ...
    # A is function
       ...
       etype = Float64
       dtype = qflag ? Quaternion{Float64} : ComplexF64
    end
    ...
    # desired level of orthogonality (the defult value)
    delta = delta == [] ? sqrt(smleps/k) : delta

    # critia for selecting subset of vectors
    eta   = etype(smleps^(3.0 / 4.0) / sqrt(k))

    # the estimated error of matrix multiplication
    eps1  = etype(sqrt(maximum([m n])) * smleps / 2.0)
    ...
    elseif Uold != [] && Bold != [] && Vold != []
       ...
       (p, beta[j0+1], rr) = reorthogonalization(U, p, beta[j0+1], indexSet, gamma=gamma, iflag=gs_flag)
    ...
    for j = j0 : k
        ...
        if j == 6
           Btmp = vcat(SparseArrays.diagm(0=>alpha[1:j-1])+SparseArrays.diagm(-1=>beta[2:j-1]), zeros(1, j-1))
           Btmp[j,j-1] = beta[j]
...
#    k = j < k ? j : k
    Bk = SparseArrays.spdiagm(0 => alpha[1:k], -1 => beta[2:k])
    #    Bk = SparseArrays.spdiagm((alpha[1:k], beta[2:k+1]), (0,-1), k+1, k)
    # if beta[k+1] > 0
...
"""
function reorthogonalization(Q::Matrix{Tv}, r::Vector{Tv}, normold::T,
                             index::Vector{Ti}, gamma::T, iflag::Ti) where {Tv<:Number, T <: AbstractFloat, Ti<:Int64}
...
function updateIptU(iptU_old::Vector{Tv}, iptV::Vector{Tv}, j::Ti,
    ...
    etype    = eltype(alpha)
    iptU_new = zeros(Tv, length(iptU_old))
...
function updateIptV(iptV_old::Vector{Tv}, iptU::Vector{Tv}, j::Ti,
    ...
    iptV_new = zeros(Tv, length(iptV_old))
...
function selectVectors(ipt::Vector{Tv}, j::Ti, delta::Tv, eta::Tv,
    ...
    if strategy == 0
       ...
       I0 = findall(x->(abs(x) >= delta), ipt[1:j])
       if length(I0) == 0
       ...
       index = findall(x->(x>0), index)

    elseif strategy == 1

       I0 = findall(x->(abs(x) > eta), ipt[1:j])
       ...
       index = findall(x->(abs(x) > eta), ipt[1:j])
